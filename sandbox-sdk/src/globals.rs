...
fn gnames_addr() -> Option<*const c_void> {
    ...
    // }
    //
    // The `Data` pointer points to a list of pointers, which then point to the FNameEntries.
    //
    // Data might point to a location in memory that looks like the following:
    //
    // 0x001ef00 0x001ef22 ...
    //
    // These pointers then point to the FNameEntries themselves. In the case of Rocket League,
    // the first FNameEntries are packed next to one another. For example:
    //
    // 0x18 bytes "None\0" 0x18 bytes "ByteProperty\0" ...
    //
    // So we work backwards to determine the address of the global names.
    //
    // We first search for None and ByteProperty, using a pattern generated by `name_pattern`.
    // We then use the address found of the first pattern to search for pointers to that location.
    // This gives us another pointer. If we find that pointer in global memory, that is the `Data`
    // field of the global names, which means we found the global names.
    //
    // One issue that occurs is that most of this memory is only accessible through virtual pages.
    // This means we need to iterate through the virtual pages to search for this memory.
    // The `pages` function handles this.
    //
    // Another issue that occurs is that the second pointer occurs multiple times in the virtual pages.
    // We handle this by checking each pointer in the global memory, which is accessed through the `memory`
    // function. Once we find a pointer that has a match in global memory, we have found the correct pointer
    // and the global names.

    // First, we generate the pattern we search for to find the location of the "None" and "ByteProperty" FNameEntries.
    // This starts with "None", not the padding before it, so we will need to offset this by the padding later.
    let fnameentry = name_pattern();

    // We then search virtual memory by iterating through the allocated pages to find the generated pattern.
    // Once we find the pattern, we subtract the FNameEntry padding bytes to get the start of the FNameEntries.
    let fnameentry = pages().find_map(|mem| find_pattern(&fnameentry, mem))?;
    let fnameentry = unsafe { fnameentry.sub(FNAMEENTRY_PADDING) };

    // We then search for pointers to the address we found previously. In order to search for the address,
    // we convert the pointer into its constitutent bytes.
    // In order to optimize the page search, we create a finder using the `memchr` crate.
    ...
    // This step takes the largest amount of time. There are multiple instances of the address we are searching
    // for, but only one of them will be found in the global memory, which will be the address of the global names.
    //
    // We first create an iterator that searches through pages and returns the address of any pointers if they are
    // found. We search through pages using the `memmem::Finder` to optimize this process.
    //
    // Next we search global memory for the bytes of the pointer we just found. Once we find a match, we have found
    // the global names `TArray`.
    pages()
        .filter_map(|mem| {
            fnameentry
                .find(mem)
